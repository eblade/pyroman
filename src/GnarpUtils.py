...
# Utils for common tasks in the Gnarp report processor
...
def varsub(_text, _dicts, _templates):
    ...
    for m in  m_var:
        ...
        if not template:
            G.crit(''.join(['Template not found: ',m[2]]))
            next
        result = u''
